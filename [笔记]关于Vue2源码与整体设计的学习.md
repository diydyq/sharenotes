...
从上面`代码块：3. 对属性值为obj字典对象的属性代理`的代码中，我们可以看到：对象obj中的每个属性key原本的值val都会重新以**defineProperty()**的方式重新定义；同时针对每个属性`key`，都会以闭包的形式定义对应的Dep实例`dep`，那么使用dep在getter时收集依赖方，setter时通知依赖方是不是一种很好的方式呢？
